...
class NonPkptActivity extends Model
{
    ...
    {
        static::creating(function (NonPkptActivity $activity): void {
            $activity->created_by ??= auth()->id();
            $activity->updated_by ??= auth()->id();
        });

        static::updating(function (NonPkptActivity $activity): void {
    ...
    {
        return $this->belongsToMany(TeamMember::class, 'non_pkpt_activity_team_member')
            ->withPivot('role')
            ->withTimestamps();
    }
